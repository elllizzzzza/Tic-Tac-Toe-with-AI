...
public class AiPlayerMedium implements Player {
    ...
    public void makeMove(final char[][] board) {
        ...
        if (tryToWinOrBlock(board, opponent)) {
            return;
        }
        fallbackMove(board);
    }

    private boolean tryToWinOrBlock(final char[][] board, final char s) {
    ...
    private boolean checkLine(final char a, final char b, final char c, final char s) {
        int countS = 0;
        int countEmpty = 0;
    ...
    private void fallbackMove(final char[][] board) {
        int x, y;
